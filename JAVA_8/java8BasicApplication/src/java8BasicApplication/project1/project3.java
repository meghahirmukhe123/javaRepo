...
class stu
{
	...
	public stu(String name,String lname,int marks,int std,LocalDate addate,ZonedDateTime zone2)
	{
		this.name=name;
		this.lname=lname;
		this.marks=marks;
...
public interface project3 {
	public static void main(String[] args)
	{
		...
		for(int i=0;i<size;i++)
		{
			...
			System.out.println("enter "+name+"'s zone: ");
			String country=sc.next();
			ZonedDateTime zone=ZonedDateTime.of(LocalDateTime.now(), ZoneId.of(country));

		
			
			l.add(new stu(name,lname,marks,std,addate,zone));
			System.out.println();
		}
		
		//lets check whether student in attending lectures online or from classroo.
		//if zone is asia/kolkata = from classroom
		//if zone != asia/kolkata=online classess
		
		for(stu s:l)
		{
			if(s.zone.equals( "Asia/Kolkata"))
			{
				System.out.println(s.name+" is attending live classess");
				System.out.println();
			}
			else {
				System.out.println(s.name+" is from "+s.zone+" so he/she is attending classess online");
				System.out.println();
		...
		for(stu s:l)
		{
			...
			{
				newstdcount+=1;
				System.out.println("name is: "+s.name+" last name: "+s.lname+" marks: "+s.marks+" old std is: "+s.std+" new standard is: "+f.apply(s)+" date of admission is: "+s.addate2);
				System.out.println();
			}
		}
